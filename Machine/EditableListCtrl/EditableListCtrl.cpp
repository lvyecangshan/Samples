...
void CEditableListCtrl::ShowCombo(bool bShow, int nItem, int nSubItem, CRect rcCtrl)
{
	...
	if (bShow == TRUE && !m_Col2ComboValue[nSubItem].empty())
	{
		...
		{
			CStringA astr1(iterValue->c_str());
			m_combo.AddString(CString(astr1));
		}
...
void CEditableListCtrl::setCell( int row,int col,CString cstr )
{
	SetItemText(row,col,cstr);
}

CString CEditableListCtrl::getCell( int row,int col )
{
	wchar_t charr[1024];
	memset(charr,0,2048);
	GetItemText(row,col,charr,1024);
	CString cstr=charr;
	return cstr;
}

int CEditableListCtrl::getFirstSelectLine()//获取选中的第一行的编号
{
